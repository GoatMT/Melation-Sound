...
function renderCard(profile) {
  ...
  const liked = profile.reactions.filter(item => item.type === 'like').map(item => songFor(item.songId)).filter(Boolean);
  const disliked = profile.reactions.filter(item => item.type === 'dislike').map(item => songFor(item.songId)).filter(Boolean);
