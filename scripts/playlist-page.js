...
function render() {
  ...
  if(owner){ const input=document.getElementById('playlistPageName'); if(document.activeElement!==input) input.value=playlist.name; const note=document.querySelector('#playlistPageImage + .playlist-editor-note'); if(note) note.textContent='Upload any image. Your cover is automatically center-cropped and adjusted to a square for fast loading.'; populateAdd(); }
...
async function savePlaylist(){if(sessionUser()?.usernameKey!==accountKey)return;const nameInput=document.getElementById('playlistPageName');const chosenName=String(nameInput?.value||'').trim();if(chosenName)playlist.name=chosenName.slice(0,50);playlist.updatedAtMs=Date.now();if(pendingPlaylistImage)playlist.imageData=pendingPlaylistImage;try{await saveAccount({playlist});pendingPlaylistImage='';setStatus('Playlist saved.');render();}catch(error){setStatus('Could not save playlist. Publish the current Firestore rules and try again.',true);}}
async function readImage(file,maxOutput){return new Promise((resolve,reject)=>{const url=URL.createObjectURL(file);const image=new Image();image.onload=()=>{URL.revokeObjectURL(url);const size=Math.max(1,Math.min(maxOutput,Math.max(image.naturalWidth,image.naturalHeight)));const canvas=document.createElement('canvas');canvas.width=size;canvas.height=size;const context=canvas.getContext('2d');const sourceSize=Math.min(image.naturalWidth,image.naturalHeight);const sourceX=(image.naturalWidth-sourceSize)/2;const sourceY=(image.naturalHeight-sourceSize)/2;context.drawImage(image,sourceX,sourceY,sourceSize,sourceSize,0,0,size,size);resolve(canvas.toDataURL('image/jpeg',.84));};image.onerror=()=>{URL.revokeObjectURL(url);reject(new Error('That image could not be read.'));};image.src=url;});}
function bind(){document.getElementById('playlistPlayAll').addEventListener('click',()=>playlist.trackIds.length&&playIndex(0));document.getElementById('playlistCopyLink').addEventListener('click',async()=>{const url=new URL('playlist.html',location.href);url.searchParams.set('uid',accountKey);try{await navigator.clipboard.writeText(url.href);setStatus('Playlist link copied.');}catch(error){setStatus(url.href);}});document.getElementById('playlistPageSave').addEventListener('click',savePlaylist);document.getElementById('playlistPageAddButton').addEventListener('click',()=>{const id=document.getElementById('playlistPageAdd').value;if(id&&!playlist.trackIds.includes(id)){playlist.trackIds.push(id);savePlaylist();}});document.getElementById('playlistPageImage').addEventListener('change',async event=>{const file=event.target.files?.[0];if(!file)return;try{pendingPlaylistImage=await readImage(file,900);document.getElementById('playlistCover').src=pendingPlaylistImage;setStatus('Artwork adjusted to a square cover. Click Save playlist.');}catch(error){event.target.value='';setStatus(error.message,true);}});}
