...
function showFullDialog(room) { const dialog=ensureFullDialog(); const isPrivate=room.visibility==='private'; dialog.dataset.roomId=room.id; dialog.querySelector('.label-kicker').textContent=isPrivate?'Listening Rooms · host approval':'Listening Rooms · live capacity'; dialog.querySelector('#roomFullTitle').textContent=isPrivate?'This Room Is Private.':'This Room Is Full.'; dialog.querySelector('#roomFullCopy').textContent=isPrivate?room.name+' is private. Ask the host to let you in.':room.name+' has reached its '+roomCapacity(room)+' listener limit.'; dialog.querySelector('#roomFullStatus').textContent=''; dialog.hidden=false; dialog.querySelector('#roomFullAsk').onclick=()=>requestToJoin(room,dialog); dialog.querySelector('#roomFullAsk').focus(); }
...
function render() {
  ...
  list.innerHTML = rooms.length ? rooms.map((room, index) => {
    ...
    const accessLabel = room.visibility === 'private' ? '<span class="room-access room-access-private">Private room · ask to join</span>' : '';
...
async function join(id, options={}) {
  ...
  if (!options.approved && !isOwner(room) && room.visibility === 'private') { showFullDialog(room); return; }
...
async function createRoom(event) {
  ...
  const visibility = document.querySelector('input[name="roomVisibility"]:checked')?.value === 'private' ? 'private' : 'public';
  const room = { id:'room-' + createdAtMs, name, mood:document.getElementById('roomMood').value, visibility, trackIds:ids, currentIndex:0, maxListeners, host:user.displayName || user.username || 'Listener', ownerKey:getOwnerKey(), active:true, loopStartedAtMs:createdAtMs, createdAtMs, updatedAtMs:createdAtMs };
